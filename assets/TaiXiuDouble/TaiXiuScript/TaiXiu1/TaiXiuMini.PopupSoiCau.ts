...
namespace taixiumini {
    ...
    export class PopupSoiCau extends Dialog {
        ...
        private drawPage2() {
            ...
            var data = [];
            var curData = [];
            var listData = TaiXiuMiniController.instance.histories;
            if (listData.length > 100) {
                listData.splice(0, listData.length - 100);
            }
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    if (_isTai !== isTai) {
                        if (curData.length > maxItem) {
                            var curDataSub = curData.slice(0, maxItem);                    
                            data.push(curDataSub);
                            curData = curData.splice(maxItem, curData.length)
                            if (curData.length > maxItem) {
                                var curDataSub1 = curData.slice(0, maxItem);                    
                                data.push(curDataSub1);
                                curData = curData.splice(maxItem, curData.length)                                
                            }
                        }
                        data.push(curData);
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (row == 0)
                    iconXX123.setPosition(startPosX + spacingX * column, startPosY - spacingY * row - 4);
                else if (row == 1)
                    iconXX123.setPosition(startPosX + spacingX * column, startPosY - spacingY * row - 10);
                else if (row == 2)
                    iconXX123.setPosition(startPosX + spacingX * column, startPosY - spacingY * row - 16);
                else if (row == 3)
                    iconXX123.setPosition(startPosX + spacingX * column, startPosY - spacingY * row - 20);
                else
                    iconXX123.setPosition(startPosX + spacingX * column, startPosY - spacingY * row - 25);
