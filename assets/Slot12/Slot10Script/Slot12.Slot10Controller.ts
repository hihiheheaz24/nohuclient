...
import cmd from "./Slot12.Cmd";

import Configs from "../../Loading/src/Configs";
import PopupSelectLine from "./Slot12.PopupSelectLine";
import PopupBonus from "./Slot12.PopupBonus";
import TrialResults from "./Slot12.TrialResults";
import Slot10Lobby from "./Slot12.Lobby";
...
import Slot10ItemSlot from "./Slot12.Item";
...
@ccclass
export default class Slot12Controller extends cc.Component {
        ...
        if (!this.mutipleJpNode) {
            cc.assetManager.getBundle("Lobby").load("prefabs/MutipleJackpotPr", cc.Prefab, function (finish, total, item) {
                ...
                if (err1 != null) {
                    ...
                    this.mutipleJpNode.setInfo("THANDEN");
                }
            })
        }
    }
    ...
    testJackpot(){
        this.showEffectJackpot(30000000);
    }
    testBigWin(){
        this.showEffectBigWin(30000000,()=>{});
    }
    ...
    private showEffectJackpot(cash: number, cb: () => void = null) {
        ...
        // let partical = this.effectJackpot.getComponentInChildren(cc.ParticleSystem);
        // partical.resetSystem();
    ...
    showWildBig(roll) {
        ...
        for (var i = 0; i < slotDatas.length; i++) {
            if (slotDatas[i] == self.wildItemId) {
                ...
                self.iconWildColumns.children[indexRow].getComponent("sp.Skeleton").setAnimation(0, "Will-2", false);
            }
        }
