...
export class TabNapThe {
    ...
    tabSelectMang: cc.ToggleContainer = null;

    private tabSelectedNhaMang = -1;
    start() {
        this.itemFactorTemplate.active = false;
        for (let i = 0; i < this.tabSelectMang.toggleItems.length; i++) {
            this.tabSelectMang.toggleItems[i].node.on("toggle", () => {
    ...
    submit() {
        let ddTelcoValue = this.tabSelectedNhaMang;
...
export default class PopupShop extends Dialog {
    ...
    actGetOTP() {
        App.instance.showLoading(true);
        MiniGameNetworkClient2.getInstance().send(new cmd.ReqGetOTP(0));
    }
    actSubmitTransfer() {
