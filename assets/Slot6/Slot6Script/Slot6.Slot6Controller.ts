...
import Slot6Lobby from "./Slot6.Lobby";
...
export default class Slot6Controller extends cc.Component {
    ...
    public onBtnSelectRoom(room) {
        SlotNetworkClient.getInstance().send(new cmd.SendChangeRoom(this.betIdx, room));
        SlotNetworkClient.getInstance().send(new cmd.SendUnSubcribe(this.betIdx));
        this.betIdx = room;
        if (this.betIdx >= 2) {
            this.betIdx = 2;
