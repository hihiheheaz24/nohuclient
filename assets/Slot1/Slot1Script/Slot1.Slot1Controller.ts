...
export default class Slot1Controller extends cc.Component {
    ...
    public onJoinRoom(res = null) {
        ...
        this.totalBetLabel.string = Utils.formatNumber(totalbet);
    ...
    showWinEffect(prize: number, currentMoney: number, result: number) {
        ...
        if (prize > 0) {
            ...
            if (result == 5) {
                ...
                bigwinSpine.animation = "thang lon";
                ...
                jackpotSpine.animation = "no hu";
                jackpotSpine.loop = true;

                let partical = this.jackpotNode.getComponentInChildren(cc.ParticleSystem);
    ...
    testBigWin(){
        ...
        this.bigWinNode.active = true;
        let partical = this.bigWinNode.getComponentInChildren(cc.ParticleSystem);
        partical.resetSystem();
        let bigwinSpine = this.bigWinNode.getComponentInChildren(sp.Skeleton);
        bigwinSpine.animation = "thang lon";
        bigwinSpine.loop = true;
    ...
    testJackpot(){
        ...
        var partical = this.jackpotNode.getComponentInChildren(cc.ParticleSystem);
        partical.resetSystem();
        cc.Tween.stopAllByTarget(this.nodeWinJackpot);
        this.txtWinJackpot.string = "0";
        Tween.numberTo(this.txtWinJackpot, 30000000, 3.0);
        // this.nodeWinJackpot.position = cc.v3(0, -360, 0);
        this.nodeWinJackpot.active = true;
        cc.tween(this.nodeWinJackpot)
            .to(1, { position: cc.v3(0, 0, 0) })
        ...
        this.showCoins(30000000);
        Tween.numberTo(this.totalWinLabel, 30000000, 1.0);
    }
    ...
    actHistory() {
        ...
        if (this.popupHistory == null) {
            BundleControl.loadPrefabGame("Slot1", "duaxe/prefabs/PopupHistory", (finish, total) => {
                ...
                App.instance.showLoading(false);
                this.popupHistory = cc.instantiate(prefab).getComponent("Slot1.PopupHistory");
