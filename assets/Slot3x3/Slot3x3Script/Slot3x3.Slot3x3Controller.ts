...
export default class Slot3x3Controller extends MiniGame {
    ...
    setItemAnim(item, id, idle = true) {
        ...
        item.getChildByName("skeleton").getComponent(cc.Animation).stop();
        if (id < 3) {
            item.getChildByName("skeleton").active = true;
            item.getChildByName("skeleton").getComponent(cc.Animation).play();
        } else {
            item.getChildByName("skeleton").active = false;
        }
        item.getChildByName("sprite").active = true;
        item.getChildByName("sprite").getComponent(cc.Sprite).spriteFrame = this.sprFrameItems[id];
    }
    onToggleAuto() {
    ...
    private setEnabledAllButtons(isEnabled: boolean) {
        ...
        this.btnSpin.enableAutoGrayEffect = !isEnabled;
