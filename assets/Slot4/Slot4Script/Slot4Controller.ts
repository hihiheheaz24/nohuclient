...
export default class Slot4Controller extends cc.Component {
    ...
    randomIconList() {
        ...
        for (var i = 0; i < self.arrUIItemIcon.length; i++) {
            ...
            self.arrUIItemIcon[i].changeSpriteRealByItemId(itemId);
            self.arrUIItemIcon[i].spriteIcon.node.active = true;
            self.arrUIItemIcon[i].spineIcon.node.active = false;
        }
    }
    ...
    spinFinish(hasDelay: boolean) {
        ...
        this.node.runAction(
            cc.sequence(
                ...
                cc.callFunc(() => {
                    if (that.toggleFast.isChecked || this.isAutoSpin) {
                        that.spinClick();
                    } else {
    ...
    showWinEffect(prize: number, currentMoney: number, result: number) {
        ...
        if (prize > 0) {
            if (result == 5) {
                ...
                bigwinSpine.setAnimation(0,"ThangLon",false);
                this.txtWinBigWin.string = "0";
                ...
                jackpotSpine.setAnimation(0,"NoHu",true);
                this.showCoins(prize);
                this.nodeWinJackpot.active = false;
                this.txtWinJackpot.node.active = false;
                this.node.runAction(
                    cc.sequence(
                        cc.delayTime(1),
                        cc.callFunc(()=>{
                            this.txtWinJackpot.node.active = true;
    ...
    endSpinReel(indexReel) {
        ...
        if (self.dataResult == null) {
            for (var i = 3; i >= 1; i--) {
                var index = indexReel + (i * 5);
                var itemId = Math.floor(Math.random() * (self.iconSFArr100.length));
                self.arrUIItemIcon[index].changeSpriteRealByItemId(itemId);
                self.arrUIItemIcon[index].spriteIcon.node.active = true;
                self.arrUIItemIcon[index].spineIcon.node.active = false;
            }
            return;
        ...
        for (var i = 3; i >= 1; i--) {
            var index = indexReel + (i * 5);
            self.arrUIItemIcon[index].changeSpriteRealByItemId(matrix[index - 5]);
            self.arrUIItemIcon[index].spriteIcon.node.active = true;
            self.arrUIItemIcon[index].spineIcon.node.active = false;
        }
        cc.tween(self.arrReel[indexReel])
    ...
    setAutoSpin() {
        ...
        if (!this.isAutoSpin) {
            this.btnAutoSpin.node.getComponent(cc.Sprite).spriteFrame = this.iconAutoSpin[0];
        } else {
            this.btnAutoSpin.node.getComponent(cc.Sprite).spriteFrame = this.iconAutoSpin[1]; //dung
        }
   
        if (!this.isSpining && this.isAutoSpin) {
            this.spinClick();
        }
    }
